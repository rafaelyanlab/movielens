...
# You need to run the entire R script "01_movielens.R" before running this rmd file. 
```
\newpage
...
```{r loading train set}
# This piece of code considers that you've already created the edx and validation sets or you have downloaded the entire project from GitHub:
# 
...
```{r pca results, echo=T, eval = F}
pca_edx_residuals <- readRDS("rda/pca_edx_residuals.rda")
```

```{r dim objects}
dim_pca_x <- readRDS("rda/dim_pca_x.rda")
dim_pca_rotation <- readRDS("rda/dim_pca_rotation.rda")
```
...
The PCA results consist of:
  ...
  * A matrix __x__ with the coordinates of users projected into each PC and dimensions `r dim_pca_x`.
  * A matrix __rotation__ of eigenvectors and dimensions `r dim_pca_rotation`.
...
knitr::include_graphics("figs/scree_plot.png")
```
...
```{r eimpute result}
edx_em_svd_matrix_extract <- readRDS("rda/edx_em_svd_matrix_extract.rda")

edx_em_svd_matrix_extract %>%
